...
function translateNow(){
    ...
    if(originalLangObject && targetLangObject && customTargetObject){
        ...
        let replaceLanguageText = replaceLanguage(originalLangObject, targetLangObject, customTargetObject);
        
        // lưu kết quả vào file output
        fs.writeFile(resultPath, util.inspect(replaceLanguageText), (err) => {
            if (err) throw err;
           });
        // lưu thêm những kết quả lỗi
        fs.writeFile(errorResultPath, util.inspect(errorList), (err) => {
            if (err) throw err;
           });
...
function replaceLanguage(originalLangObject, targetLangObject, customTargetObject){
